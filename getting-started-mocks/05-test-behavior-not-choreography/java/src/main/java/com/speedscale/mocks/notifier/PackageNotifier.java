...
public final class PackageNotifier {
    ...
    /**
     * Doubles the wait after each failed attempt, so a struggling carrier is
     * not hammered. This is the change that breaks the over-specified test in
     * this post while leaving every observable outcome alone.
     */
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            try {
                ...
                if (attempt < MAX_ATTEMPTS) {
                    sleep.pause(backoffFor(attempt));
                }
            }
        }
        throw lastFailure;
