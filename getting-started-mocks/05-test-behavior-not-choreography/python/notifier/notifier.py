...
def backoff_for(attempt: int) -> float:
    ...
    A struggling carrier is not hammered. This is the change that breaks the
    over-specified test in this post while leaving every observable outcome
    alone.
    """
        ...
        for attempt in range(1, MAX_ATTEMPTS + 1):
            try:
                ...
                if attempt < MAX_ATTEMPTS:
                    self._sleep(backoff_for(attempt))
        raise last_failure
