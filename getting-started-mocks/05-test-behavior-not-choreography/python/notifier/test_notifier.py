...
class PackageNotifierTest(unittest.TestCase):
    ...
    @unittest.skip(
        "over-specified: it asserts the exact backoff schedule, so a harmless change to backoff breaks it"
    )
    def test_give_up_choreography(self) -> None:
        """Kept on purpose as the cautionary example in this post.

        It passed until the backoff became exponential. Nothing a customer can
        observe changed: the carrier is still asked three times, nothing is
        still sent, and the same failure still surfaces. The test broke because
        it pinned how long the notifier waits between attempts, which is
        choreography, not behavior.
        """
        events: list[str] = []
    ...
    def test_giving_up_tells_nobody_and_surfaces_the_failure(self) -> None:
        ...
        It keeps exactly one interaction assertion, the one a customer could
        notice: no message was sent. How many times we retried and how long we
