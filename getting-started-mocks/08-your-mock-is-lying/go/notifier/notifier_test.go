...
func TestContractFailureIsNotRetried(t *testing.T) {
	calls := 0
	drifted := func(string) (string, error) {
		calls++
		return "", ErrCarrierContract
	}
	n := New(drifted, (&spySender{}).Send, NewMemoryRecorder(), dummySleeper)
	if err := n.Notify("TRACK-123"); !errors.Is(err, ErrCarrierContract) {
		t.Fatalf("expected contract error, got %v", err)
	}
	if calls != 1 {
		t.Fatalf("contract error was retried %d times", calls)
	}
}
