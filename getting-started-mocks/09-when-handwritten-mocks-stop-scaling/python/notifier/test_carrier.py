...
def fixture(name: str) -> str:
    return (Path(__file__).parents[2] / "fixtures" / name).read_text()
...
class CarrierClientTest(unittest.TestCase):
    ...
    def test_parses_the_captured_fixture(self) -> None:
        with FakeCarrier(body=fixture("carrier-shipment-delayed.json")) as carrier:
            self.assertEqual(
                "delayed", CarrierClient(carrier.url, "secret-key").lookup("TRACK-123")
            )
    ...
    def test_rejects_the_renamed_field(self) -> None:
        with FakeCarrier(body=fixture("carrier-shipment-delayed-v2.json")) as carrier:
            with self.assertRaises(CarrierContractError) as caught:
                CarrierClient(carrier.url, "secret-key").lookup("TRACK-123")
        self.assertNotIsInstance(caught.exception, CarrierUnavailableError)
