...
echo "  Minikube        : ./deploy-local.sh tunnel  then  http://localhost  (via ingress)"
echo ""
echo "  Note: minikube tunnel is required for the ingress to receive traffic."
echo "  Traffic flows Browser → nginx ingress → pod, so Speedscale sidecar"
echo "  captures inbound calls on eth0 (not loopback/tunnel bypass)."
echo ""
