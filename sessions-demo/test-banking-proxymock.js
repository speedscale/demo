  ...
  try {
    ...
    if (validateGroups && process.env.BANK_KEEP_RUNNING === '1') {
      const manual = { base, artifacts, binary, controlToken: driverOptions.controlToken,
        recording: path.join(artifacts, ({ synthesis: 'inbound-synthesis', clones: 'inbound-clones', workers: 'inbound-requests' })[process.env.BANK_LOAD_CASES] || 'inbound-sessions'), plan: path.join(artifacts, `${({ composition: 'composition-sessions', multiples: 'multiples-sessions', goals: 'goals-session-endpoint', identity: 'identity-rotation', synthesis: 'synthesis-rotation', clones: 'clones-once', workers: 'workers-mixed' })[process.env.BANK_LOAD_CASES] || 'sessions-rotate'}-plan.json`) };
